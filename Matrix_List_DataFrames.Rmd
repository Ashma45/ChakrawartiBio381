...
```{r}
sq_m <- matrix(data = fx, nrow = sqrt(length(fx))) # using sqaure root of the length of vector for nrow
```
...
```{r}
eg<- eigen(sq_mt, symmetric = TRUE, only.values = FALSE, EISPACK = FALSE) # calculates eigenvalues and eigenvectors of matrix
```
