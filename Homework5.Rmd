...
```{r}
# new data set with length as my original data and mean 33.98

z1 <- rnorm(n=116,mean=33.98,sd=12.09)
z1 <- data.frame(1:116,z1)
...
``` 
The histograms for my original dataset and the simulated new dataset do not match with each other. The model is not predicting the realistic data. This might be because the parameters like mean and sd were specified beforehand for the simulated data to which the best fitting normal distribution data was created.
```
...
```{r}
expoPars <- fitdistr(z1$myVar,"exponential")
rateML <- expoPars$estimate["rate"]

stat2 <- stat_function(aes(x = xval, y = ..y..), fun = dexp, colour="blue", n = length(z1$myVar), args = list(rate=rateML))
 p2 + stat + stat2
```
